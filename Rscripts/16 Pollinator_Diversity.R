...
dat<- read.csv ("Data/datPollinators.csv", header=TRUE, sep=",",na.strings=TRUE)
...
# the effect of spatial heterogeneity (i.e. all crop and landscape heterogeneity components) on pollinator diversity
mod1 <- rma.mv(yi, vi, random = ~ 1 | Study_ID / Effect_Size_ID, data=dat, subset=(Biodiversity_Category=="SWDiversity"), method="REML",level=95)
summary(mod1)
...
# the effects of spatial compositional heterogeneity (i.e. crop and landscape compositional heterogeneity components) and spatial configurational heterogeneity (i.e. crop and landscape configurational heterogeneity components) on pollinator diversity
mod2 <- rma.mv(yi, vi, mods = ~ factor(Heterogeneity) - 1, random = ~ 1 | Study_ID / Effect_Size_ID, data=dat, subset=(Biodiversity_Category=="SWDiversity"), method="REML",level=95)
summary(mod2)
...
table(dat$Heterogeneity)
Compo <- rma.mv(yi, vi, random = ~ 1 | Study_ID / Effect_Size_ID, data=dat, subset=(Biodiversity_Category=="SWDiversity")&(Heterogeneity=="Compositional_het"), method="REML",level=95)
summary (Compo)

table(dat$Heterogeneity)
Config <- rma.mv(yi, vi, random = ~ 1 | Study_ID / Effect_Size_ID, data=dat, subset=(Biodiversity_Category=="SWDiversity")&(Heterogeneity=="Configurational_het"), method="REML",level=95)
summary (Config)

########################################################################################################################################################################################

# the effects of crop compositional heterogeneity, crop configurational heterogeneity, landscape compositional and landscape configurational heterogeneity on pollinator diversity
mod3 <- rma.mv(yi, vi, mods = ~ factor(Heterogeneity_Type) - 1, random = ~ 1 | Study_ID / Effect_Size_ID, data=dat, subset=(Biodiversity_Category=="SWDiversity"), method="REML",level=95)
summary(mod3)
...
# this is just to count how many studies and effect sizes included in each test
table(dat$Heterogeneity_Type)
CropCompo <- rma.mv(yi, vi, random = ~ 1 | Study_ID / Effect_Size_ID, data=dat, subset=(Biodiversity_Category=="SWDiversity")&(Heterogeneity_Type=="Crop_compositional_het"), method="REML",level=95)
summary (CropCompo)

CropConfig <- rma.mv(yi, vi, random = ~ 1 | Study_ID / Effect_Size_ID, data=dat, subset=(Biodiversity_Category=="SWDiversity")&(Heterogeneity_Type=="Crop_Configurational_het"), method="REML",level=95)
summary (CropConfig)

LandscapeCompo <- rma.mv(yi, vi, random = ~ 1 | Study_ID / Effect_Size_ID, data=dat, subset=(Biodiversity_Category=="SWDiversity")&(Heterogeneity_Type=="Landscape_compositional_het"), method="REML",level=95)
summary (LandscapeCompo)

LandscapeConfig <- rma.mv(yi, vi, random = ~ 1 | Study_ID / Effect_Size_ID, data=dat, subset=(Biodiversity_Category=="SWDiversity")&(Heterogeneity_Type=="Landscape_Configurational_het"), method="REML",level=95)
summary (LandscapeConfig)

########################################################################################################################################################################################

# the effects of crop heterogeneity (i.e. crop compositional and configurational heterogeneity components) and landscape heterogeneity (i.e. landscape compositional and configurational heterogeneity components) on pollinator diversity
mod4 <- rma.mv(yi, vi, mods = ~ factor(Heterogeneity_Type_L_C) - 1, random = ~ 1 | Study_ID / Effect_Size_ID, data=dat, subset=(Biodiversity_Category=="SWDiversity"), method="REML",level=95)
summary(mod4)
...
# this is just to count how many studies and effect sizes included in each test
table(dat$Heterogeneity_Type_L_C)
CropHet <- rma.mv(yi, vi, random = ~ 1 | Study_ID / Effect_Size_ID, data=dat, subset=(Biodiversity_Category=="SWDiversity")&(Heterogeneity_Type_L_C=="Crop_het"), method="REML",level=95)
summary (CropHet)

table(dat$Heterogeneity_Type_L_C)
LandHet <- rma.mv(yi, vi, random = ~ 1 | Study_ID / Effect_Size_ID, data=dat, subset=(Biodiversity_Category=="SWDiversity")&(Heterogeneity_Type_L_C=="Landscape_het"), method="REML",level=95)
summary (LandHet)
