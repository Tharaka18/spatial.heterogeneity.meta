...
dat<- read.csv ("Data/datCroppingSystems.csv", header=TRUE, sep=",",na.strings=TRUE)
...
table(dat$Annual_perennial_mixed, dat$Biodiversity_Category)

########################################################################################################################################################################################

# the effect of spatial heterogeneity (i.e. all crop and landscape heterogeneity components) on animal (i.e. invertebrate and vertebrate without pests) richness in different cropping systems
mod1 <- rma.mv(yi, vi, mods = ~ factor(Annual_Perennial) - 1, random = ~ 1 | Study_ID / Effect_Size_ID, data=dat, subset=(Biodiversity_Category=="Richness")&(Animals_Plants=="Animals"), method="REML",level=95)
summary(mod1)
...
# test difference between the two outcomes (Annual and Perennial)
...
table(dat$Annual_Perennial)
Ann <- rma.mv(yi, vi, random = ~ 1 | Study_ID / Effect_Size_ID, data=dat, subset=(Biodiversity_Category=="Richness")&(Animals_Plants=="Animals")&(Annual_Perennial=="Annual"), method="REML",level=95)
summary (Ann)

table(dat$Annual_Perennial)
Per <- rma.mv(yi, vi, random = ~ 1 | Study_ID / Effect_Size_ID, data=dat, subset=(Biodiversity_Category=="Richness")&(Animals_Plants=="Animals")&(Annual_Perennial=="Perennial"), method="REML",level=95)
summary (Per)

########################################################################################################################################################################################

# the effect of spatial heterogeneity (i.e. all crop and landscape heterogeneity components) on animal (i.e. invertebrate and vertebrate without pests) abundance in different cropping systems 
mod2 <- rma.mv(yi, vi, mods = ~ factor(Annual_Perennial) - 1, random = ~ 1 | Study_ID / Effect_Size_ID, data=dat, subset=(Biodiversity_Category=="Abundance")&(Animals_Plants=="Animals"), method="REML",level=95)
summary(mod2)
...
# this is just to count how many studies and effect sizes included in each test
table(dat$Annual_Perennial)
Ann <- rma.mv(yi, vi, random = ~ 1 | Study_ID / Effect_Size_ID, data=dat, subset=(Biodiversity_Category=="Abundance")&(Animals_Plants=="Animals")&(Annual_Perennial=="Annual"), method="REML",level=95)
summary (Ann)

table(dat$Annual_Perennial)
Per <- rma.mv(yi, vi, random = ~ 1 | Study_ID / Effect_Size_ID, data=dat, subset=(Biodiversity_Category=="Abundance")&(Animals_Plants=="Animals")&(Annual_Perennial=="Perennial"), method="REML",level=95)
summary (Per)

########################################################################################################################################################################################

# the effect of spatial heterogeneity (i.e. all crop and landscape heterogeneity components) on animal (i.e. invertebrate and vertebrate without pests) diversity in different cropping systems 
mod3 <- rma.mv(yi, vi, mods = ~ factor(Annual_Perennial) - 1, random = ~ 1 | Study_ID / Effect_Size_ID, data=dat, subset=(Biodiversity_Category=="SWDiversity")&(Animals_Plants=="Animals"), method="REML",level=95)
summary(mod3)
...
# this is just to count how many studies and effect sizes included in each test
table(dat$Annual_Perennial)
Ann <- rma.mv(yi, vi, random = ~ 1 | Study_ID / Effect_Size_ID, data=dat, subset=(Biodiversity_Category=="SWDiversity")&(Animals_Plants=="Animals")&(Annual_Perennial=="Annual"), method="REML",level=95)
summary (Ann)

table(dat$Annual_Perennial)
Per <- rma.mv(yi, vi, random = ~ 1 | Study_ID / Effect_Size_ID, data=dat, subset=(Biodiversity_Category=="SWDiversity")&(Animals_Plants=="Animals")&(Annual_Perennial=="Perennial"), method="REML",level=95)
summary (Per)
...
# diversity 
comp2 <- summary(glht(mod3, linfct=cbind(contrMat(rep(1,2), type="Tukey"))), test=adjusted("none"))
summary(comp2)

# abundance  
comp3 <- summary(glht(mod2, linfct=cbind(contrMat(rep(1,2), type="Tukey"))), test=adjusted("none"))
summary(comp3)

########################################################################################################################################################################################

plot_summs(comp1,comp2, comp3,
           inner_ci_level=0.90,
           point.shape = FALSE,
           point.size = 12.5)
